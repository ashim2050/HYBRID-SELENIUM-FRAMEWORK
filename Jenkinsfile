            ...
            steps {
                script {
                    ...
                    unstash 'search-results'
                        ...
                        cat > ${WORKSPACE}/consolidated-reports/ExtentReport_Final.html <<'INDEX'
<html>
<head><title>Extent Report - Consolidated</title></head>
<body>
<h1>Extent Report - Consolidated</h1>
<p>All node reports generated by this run are linked below.</p>
<ul>
INDEX
                        find ${WORKSPACE}/consolidated-reports -name '*.html' ! -name 'ExtentReport_Final.html' | sort | while IFS= read -r f; do
                          rel=${f#${WORKSPACE}/}
                          echo "<li><a href=\"${rel}\">${rel}</a></li>" >> ${WORKSPACE}/consolidated-reports/ExtentReport_Final.html
                        done
                        cat >> ${WORKSPACE}/consolidated-reports/ExtentReport_Final.html <<'INDEX'
</ul>
</body>
...
// Archive only Extent HTML reports from this run
                archiveArtifacts artifacts: 'consolidated-reports/**/*.html',
